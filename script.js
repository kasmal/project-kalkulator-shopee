...
const SHOPEE_TARIF_2026 = {
  admin: {
    ...
    // Shopee Mall punya struktur beda — tarif admin di Mall 1-5% tapi ada biaya
    // pembayaran 1,8% dan biaya layanan yang lebih tinggi. Untuk simplifikasi kami
    // pakai angka konservatif (admin lebih rendah + biaya pembayaran include di admin).
    "mall": {
      "A": 6.5,
      "B": 5.5,
      "C": 4.5,
...
// Animate number counter for "wow" feel when calculator updates
const animateState = new Map(); // elementId → { current, target, rafId }

function animateValue(elementId, targetValue, asRupiah = false) {
  const el = document.getElementById(elementId);
  if (!el) return;

  if (!isFinite(targetValue) || targetValue < 0) {
    el.textContent = asRupiah ? "—" : String(targetValue);
    return;
  }

  // Fallback jika requestAnimationFrame tidak tersedia (misal jsdom tanpa pretendToBeVisual)
  if (typeof requestAnimationFrame === "undefined") {
    el.textContent = asRupiah ? fmtRupiah(targetValue) : Math.round(targetValue).toLocaleString("id-ID");
    return;
  }

  const state = animateState.get(elementId) || { current: 0 };

  // Cancel previous animation
  if (state.rafId) cancelAnimationFrame(state.rafId);

  const startValue = state.current || 0;
  const duration = 600; // ms
  const startTime = performance.now();

  // Pop class for visual feedback
  ...
  const tick = (now) => {
    ...
    el.textContent = asRupiah ? fmtRupiah(value) : Math.round(value).toLocaleString("id-ID");

    if (progress < 1) {
      state.rafId = requestAnimationFrame(tick);
      animateState.set(elementId, state);
    } else {
...
function renderSingleResult() {
  ...
  animateValue("result-harga-jual", r.hja, true);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Auto-show wizard untuk user baru (belum ada profile tersimpan)
  if (!hasDefaultProfile()) {
    // Delay sedikit supaya UI sudah rendering
...
/* =========================================================================
 * HERO DATE: tampilkan tanggal hari aktif dalam bahasa Indonesia
 * ========================================================================= */
...
/* =========================================================================
 * QUOTES: rotating inspirational quotes untuk seller/bisnis
 *
 * Quote rotate setiap 10 detik. User bisa klik quote untuk skip ke next.
 * Quote dipilih random dari array tanpa repeat langsung.
 * ========================================================================= */

const QUOTES = [
  { text: "Profit bukan hasil dari harga tinggi, tapi dari hitungan yang benar.", author: "Ilmu Jualan" },
  { text: "Jangan takut menaikkan harga — takutlah merugi diam-diam.", author: "— Seller Bijak" },
  { text: "Pelanggan loyal bukan karena harga murah, tapi karena pengalaman yang konsisten.", author: "— Prinsip UMKM" },
  { text: "Yang membedakan toko sukses dari yang gagal adalah disiplin mengecek margin.", author: "— Warren Buffett (diadaptasi)" },
  { text: "Modal boleh kecil, hitungan harus besar.", author: "— Pedagang Lama" },
  { text: "Jualan itu ilmu matematika sosial. Angka harus tepat, cara harus ramah.", author: "— Guru Bisnis" },
  { text: "Satu pesanan yang rugi lebih berbahaya dari seratus pesanan yang untung tipis.", author: "— Pelajaran Dagang" },
  { text: "Boncos itu bukan nasib. Boncos itu kalkulator yang tidak dipakai.", author: "— Analis Shopee" },
  { text: "Iklan tanpa hitung-hitungan = membuang uang sambil berharap keajaiban.", author: "— Digital Marketer" },
  { text: "Yang disiplin menghitung hari ini, jadi pengusaha besok.", author: "— Sawyer T. Jones" },
  { text: "Toko kecil yang tahu ROAS-nya > toko besar yang asal iklan.", author: "— Founder E-commerce" },
  { text: "Harga jual yang benar = menghargai kerja keras Anda sendiri.", author: "— Filosofi Seller" },
  { text: "Bisnis yang bertahan bukan yang paling besar, tapi yang paling teliti.", author: "— Evolusi Usaha" },
  { text: "Setiap transaksi adalah ujian kecil: apakah Anda untung, atau hanya sibuk?", author: "— Prinsip Profit" },
  { text: "Gratis ongkir itu bukan gratis. Someone pays — pastikan itu bukan Anda diam-diam.", author: "— Pelajaran XTRA" },
  { text: "Kreativitas tanpa kalkulator = karya indah yang jual rugi.", author: "— Seniman Wirausaha" },
];
...
function setupQuotes() {
  const container = document.getElementById("hero-quote");
  if (!container) return;

  // Initial quote
  showRandomQuote();

  // Rotate setiap 12 detik
  quoteRotateTimer = setInterval(showRandomQuote, 12000);
...
function showRandomQuote() {
  ...
  // Pick random index yang berbeda dari current
  let idx;
  do {
...
/* =========================================================================
 * WIZARD: 5-step guided setup untuk user baru
 *
 * Alur:
 *   Step 1: Level toko (Non-Star / Star / Star+ / Mall)
 *   Step 2: Kategori produk (A/B/C/D/E/Khusus)
 *   Step 3: Gratis Ongkir (ya/tidak/belum tau)
 *   Step 4: Live/Promo XTRA (tidak/live/promo/keduanya)
 *   Step 5: HPP + Target profit
 *
 * State: wizardState object menyimpan pilihan user, apply di akhir saat "Selesai".
 * ========================================================================= */

const STORAGE_KEY = "shopee_kalkulator_default_profile_v2";
...
let wizardState = {
  ...
  answers: {
    ...
    "kategori": null,
    "prog-ongkir": null,
    "combo-45": "none", // "none" | "live" | "promo" | "both"
    "hpp": null,
    "profit": null,
  },
};
...
function setupWizard() {
  ...
  if (skipBtn) skipBtn.addEventListener("click", () => {
    closeWizard();
    showToast("Oke, isi manual saja di form kalkulator", "default");
  });
  ...
  // Option clicks (steps 1-4)
  document.querySelectorAll(".wizard-option").forEach(btn => {
    btn.addEventListener("click", () => handleWizardOption(btn));
  });

  // Chip clicks (step 5 profit suggestions)
  document.querySelectorAll(".wizard-chip").forEach(chip => {
    chip.addEventListener("click", () => {
      const val = chip.dataset.wProfit;
      const input = document.getElementById("wizard-profit");
      if (input && val) input.value = val;
      document.querySelectorAll(".wizard-chip").forEach(c => c.classList.remove("wizard-chip--selected"));
      chip.classList.add("wizard-chip--selected");
    });
  });

  // Auto-format HPP input in wizard
  const hppInput = document.getElementById("wizard-hpp");
  if (hppInput) {
    hppInput.addEventListener("input", (e) => {
...
function openWizard() {
  ...
  // Reset UI
  document.querySelectorAll(".wizard-option").forEach(b => b.classList.remove("wizard-option--selected"));
  document.querySelectorAll(".wizard-chip").forEach(c => c.classList.remove("wizard-chip--selected"));
  const hppInput = document.getElementById("wizard-hpp");
  const profitInput = document.getElementById("wizard-profit");
...
function showWizardStep(n) {
  ...
  document.querySelectorAll(".wizard-step").forEach(s => {
    s.classList.toggle("wizard-step--active", Number(s.dataset.step) === n);
  });

  // Update progress indicators
  document.querySelectorAll(".wizard-progress__step").forEach(el => {
    const step = Number(el.dataset.step);
    el.classList.remove("wizard-progress__step--active", "wizard-progress__step--done");
    if (step < n) el.classList.add("wizard-progress__step--done");
    else if (step === n) el.classList.add("wizard-progress__step--active");
  });

  // Update progress bars
  const bars = document.querySelectorAll(".wizard-progress__bar");
  bars.forEach((bar, idx) => {
    // bar[0] is between step 1 and 2, etc.
    bar.classList.toggle("wizard-progress__bar--done", idx < n - 1);
  });

  // Show/hide buttons
  document.getElementById("wizard-back").hidden = n === 1;
  const isLast = n === wizardState.totalSteps;
  document.getElementById("wizard-next").hidden = isLast;
  document.getElementById("wizard-finish").hidden = !isLast;
}

function handleWizardOption(btn) {
  const currentStep = wizardState.currentStep;

  // Mark as selected (visual)
  const stepContainer = btn.closest(".wizard-step");
  stepContainer.querySelectorAll(".wizard-option").forEach(b => b.classList.remove("wizard-option--selected"));
  btn.classList.add("wizard-option--selected");

  // Store answer
  const field = btn.dataset.wField;
  const flag = btn.dataset.wFlag;
  const combo = btn.dataset.wCombo;
  const value = btn.dataset.wValue;

  if (field) {
    wizardState.answers[field] = value;
  } else if (flag) {
    wizardState.answers[flag] = value === "true";
  } else if (combo) {
    wizardState.answers["combo-45"] = combo;
  }

  // Auto-advance for steps 1-4 (except step 5 has inputs)
  if (currentStep < 5) {
...
function wizardNext() {
  const n = wizardState.currentStep;

  // Validate current step
  if (n === 1 && !wizardState.answers["level-toko"]) {
    showToast("Pilih level toko dulu", "error");
    return;
  }
  if (n === 2 && !wizardState.answers["kategori"]) {
    showToast("Pilih kategori produk dulu", "error");
    return;
  }
  if (n === 3 && wizardState.answers["prog-ongkir"] === null) {
    showToast("Pilih salah satu jawaban Gratis Ongkir", "error");
    return;
  }
  // Step 4 always has default "none" so no validation needed
  if (n === 5) {
    // Handled by wizardFinish
    return;
  }
...
function wizardFinish() {
  ...
  const hppInput = document.getElementById("wizard-hpp");
  const profitInput = document.getElementById("wizard-profit");
  const hppVal = parseMoney(hppInput.value);
  const profitVal = parseNumber(profitInput.value);

  if (hppVal <= 0) {
    showToast("HPP wajib diisi supaya kalkulator akurat", "error");
    hppInput.focus();
    return;
  }
  if (profitVal <= 0) {
    showToast("Target profit wajib diisi", "error");
    profitInput.focus();
    return;
  }

  wizardState.answers["hpp"] = hppVal;
  wizardState.answers["profit"] = profitVal;

  // Apply to kalkulator
  applyWizardToCalculator();

  closeWizard();
  showToast("✓ Setup selesai! Lihat harga jual yang disarankan.", "success");
}

function applyWizardToCalculator() {
  const a = wizardState.answers;
  const highlightIds = [];

  // Step 1 — Level toko
  const levelSelect = document.getElementById("level-toko");
  if (levelSelect && a["level-toko"]) {
    levelSelect.value = a["level-toko"];
    levelSelect.classList.add("highlight-change");
    highlightIds.push("level-toko");
  }

  // Step 2 — Kategori (+ apply admin preset)
  const katSelect = document.getElementById("kategori");
  if (katSelect && a["kategori"]) {
    katSelect.value = a["kategori"];
    katSelect.classList.add("highlight-change");
    highlightIds.push("kategori");
  }
  applyAdminPreset();
  const adminEl = document.getElementById("admin");
  if (adminEl) {
    adminEl.classList.add("highlight-change");
    highlightIds.push("admin");
  }

  // Step 3 — Gratis Ongkir
  setProgramToggle("prog-ongkir", "xtra-ongkir", a["prog-ongkir"]);
  if (a["prog-ongkir"]) highlightIds.push("xtra-ongkir");

  // Step 4 — Live / Promo combos
  const combo = a["combo-45"];
  setProgramToggle("prog-live", "xtra-live", combo === "live" || combo === "both");
  setProgramToggle("prog-promo", "xtra-promo", combo === "promo" || combo === "both");
  if (combo === "live" || combo === "both") highlightIds.push("xtra-live");
  if (combo === "promo" || combo === "both") highlightIds.push("xtra-promo");

  // Step 5 — HPP & profit
  const hppEl = document.getElementById("hpp");
  if (hppEl && a.hpp) {
    hppEl.value = Math.round(a.hpp).toLocaleString("id-ID");
    hppEl.classList.add("highlight-change");
    highlightIds.push("hpp");
  }
  const profitEl = document.getElementById("profit");
  if (profitEl && a.profit) {
    // Pastikan mode % aktif
    const modeSelect = document.getElementById("profit-mode");
    if (modeSelect && modeSelect.value !== "percent") {
      modeSelect.value = "percent";
      handleProfitModeChange();
    }
    profitEl.value = String(a.profit).replace(".", ",").replace(/,0$/, "");
    profitEl.classList.add("highlight-change");
    highlightIds.push("profit");
  }
...
// Helper: aktif/nonaktif program dengan tarif default
function setProgramToggle(toggleId, inputId, isOn) {
  const toggle = document.getElementById(toggleId);
  ...
  if (isOn) {
    ...
    const val = defaults[inputId] || 0;
    input.value = String(val).replace(".", ",").replace(/,0$/, "");
  } else {
    input.value = "0";
  }
}
...
/* =========================================================================
 * PROFILE BUTTONS (Save/Load) — tombol di luar wizard
 * ========================================================================= */
...
function setupProfileButtons() {
  ...
  if (saveBtn) saveBtn.addEventListener("click", () => saveDefaultProfile(false));
  if (loadBtn) loadBtn.addEventListener("click", loadDefaultProfile);

  // Update UI berdasarkan apakah ada profile tersimpan
  if (loadBtn) {
    loadBtn.style.opacity = hasDefaultProfile() ? "1" : "0.5";
  }
}
...
function saveDefaultProfile(silent) {
  const profile = {
    ...
    profitMode: document.getElementById("profit-mode").value,
  ...
  try {
    ...
    if (!silent) {
      showToast("✓ Profil toko tersimpan. Bisa dipanggil lagi kapan saja.", "success");
    }
    // Enable tombol load
    const loadBtn = document.getElementById("load-default-btn");
    if (loadBtn) loadBtn.style.opacity = "1";
  } catch (e) {
    if (!silent) showToast("Gagal menyimpan: " + e.message, "error");
  }
}
...
function loadDefaultProfile() {
  try {
    ...
    if (!raw) {
      showToast("Belum ada profil tersimpan. Save dulu atau pakai Wizard.", "error");
      return;
    }
    ...
    const simple = {
      ...
      "hpp": profile.hpp,
      "profit-mode": profile.profitMode,
    ...
    // Trigger profit mode change if needed (menyesuaikan prefix/suffix)
    if (profile.profitMode) {
      // Jangan call handleProfitModeChange karena dia akan reset value
      // Kita sudah set value, cukup update visual
      const mode = document.getElementById("profit-mode").value;
      const suffix = document.getElementById("profit-suffix");
      const input = document.getElementById("profit");
      const wrap = input.parentElement;
      if (mode === "percent") {
        if (suffix) { suffix.textContent = "%"; suffix.style.display = ""; }
        const prefix = wrap.querySelector(".input-prefix");
        if (prefix) prefix.remove();
        input.removeAttribute("data-money");
        input.setAttribute("data-number", "");
    ...
    showToast(`✓ Profil dimuat${when ? " (disimpan " + when + ")" : ""}`, "success");
  } catch (e) {
